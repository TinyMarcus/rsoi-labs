...
namespace HotelsBookingSystem.GatewayAPI.Controllers
{
    ...
    public class ReservationController : ControllerBase
    {
        ...
        [Route("")]
        public async Task<IActionResult> BookHotel([FromBody] CreateReservationRequestDto createReservationRequestDto)
        {
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    var responseHotel = await client.GetAsync("http://reservation-service:80/api/v1/hotels/" +
                    ...
                    if (responseLoyalty.StatusCode == HttpStatusCode.NotFound)
                        await client.PostAsync("http://loyalty-service:80/api/v1/loyalty", null);
                    ...
                    requestMessage.Content = JsonContent.Create(new {
                        Status = "PAID",
                        Price = price });
                    var responsePayment = await client.PostAsync("http://payment-service:80/api/v1/payment", 
                    ...
                    var responseLoyaltyPost = await client.PostAsync("http://loyalty-service:80/api/v1/loyalty", 
                        null);
                    ...
                    var responseReservationPost = await client.PostAsync("http://reservation-service:80/api/v1/reservations", 
                        reservationDto);
        ...
        public async Task<IActionResult> UnbookHotel([FromRoute][Required] Guid reservationUid)
        {
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    var responseReservationGet = await client.GetAsync("http://reservation-service:80/api/v1/reservations/" +
                                                                       reservationUid);
                    ...
                    var responseReservationDel = await client.DeleteAsync("http://reservation-service:80/api/v1/reservations/" +
                                                                          reservationUid);
                    ...
                    var responsePaymentDel = await client.DeleteAsync("http://payment-service:80/api/v1/payment/" +
                                                                      paymentUid);
                    ...
                    // Decrease loyalty
                    var responseLoyaltyDel = await client.DeleteAsync("http://loyalty-service:80/api/v1/loyalty/");
                    return NoContent();
                }
            }
            catch (Exception e)
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        [Route("{reservationUid}")]
        public async Task<IActionResult> GetReservation([FromRoute][Required] Guid reservationUid)
        {
            try
            {
                using (var client = new HttpClient())
                {
                    var username = User.FindFirst("name")?.Value;
                    client.DefaultRequestHeaders.Add("username", username);
                    var responseReservation = await client.GetAsync("http://reservation-service:80/api/v1/reservations/" +
                                                                    reservationUid);
